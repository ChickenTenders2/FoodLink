...
def update_recipe_admin():
    ...
    prep = sanitised_fields[3]
    cook = sanitised_fields[4]
    servings = sanitised_fields[5]
    if user == "":
        user = None
    try:
        recipe.update_recipe(recipe_id, name, servings, prep, cook, instructions, user)
        return jsonify({'success': True})
    except Exception as e:
        print(e)
        return jsonify({'success': False, 'error': str(e)})
...
def add_recipe_admin():
    ...
    if user == "":
        user = None
    try:
        recipe.add_recipe(name, servings, prep, cook, instructions, user)
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
