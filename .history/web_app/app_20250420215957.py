...
@app.route("/recipes/get_ingredients/<int:recipe_id>")
def get_ingredients(recipe_id):
    recipe = Recipe.query.get(recipe_id)
    if not recipe:
        return jsonify(success=False, error="Recipe not found")

    # Assuming ingredients are stored in a list of tuples like (name, quantity, unit)
    ingredients = [(ing.name, ing.quantity, ing.unit) for ing in recipe.ingredients]
