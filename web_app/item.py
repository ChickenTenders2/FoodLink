...
class item_table(database):
    ...
    def item_search(self, user_id, item_name):
        cursor = self.connection.cursor()
        # searches for an item by barcode
        query = "SELECT id, name, brand, expiry_time, default_quantity, unit FROM FoodLink.item WHERE LOWER(name) = %s AND (user_id IS NULL OR user_id = %s);"
        data = (item_name, user_id)
        cursor.execute(query, data)
        item = cursor.fetchall()
        cursor.close()
        # one item per barcode so only a single item will be returned
