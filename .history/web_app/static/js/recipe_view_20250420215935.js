...
document.getElementById("update-form").addEventListener("submit", async function(event) {
    ...
    // You might want to store this for later (e.g., global or session variable)
    window.currentRecipe = {
        ...
        cook_time: cook,
        instructions,
        ingredients: [], // we'll fill this later
        personal: true, // assume editable
        tool_ids: [],
        missing_tool_ids: []
    };

    // Fetch ingredients from server (optional if passed from backend)
    const response = await fetch(`/recipes/get_ingredients/${recipeId}`);
    const result = await response.json();

    if (result.success) {
        window.currentRecipe.ingredients = result.ingredients;
    } else {
        alert("Error loading ingredients.");
    }

    // Open the edit ingredients popup
    open_edit_ingredients_popup(window.currentRecipe.ingredients);
});